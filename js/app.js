...
const sections = {
airtime: document.getElementById('airtimeSection'),
data: document.getElementById('dataSection'),
utility: document.getElementById('utilitySection')
};
...
if (sections[service]) {
sections[service].style.display = 'block';
}

if (service === 'airtime') loadAirtimeProviders();
if (service === 'data') loadDataProviders();
if (service === 'utility') loadUtilityTypes();
}
...
if (data.success) {
  ...
  const element = document.getElementById('walletBalance');

  if (element) {
    element.textContent = `₦${balance.toLocaleString('en-NG', {
  ...
  if (currentUser) {
    currentUser.wallet_balance = balance;
    localStorage.setItem('user', JSON.stringify(currentUser));
  }
...
showMessage('fundMessage', 'Minimum funding amount is ₦100', 'error');
return;
}

try {
showMessage('fundMessage', 'Initializing payment...', 'info');
...
if (!response.ok || !data.success) {
  showMessage('fundMessage', data.message || 'Unable to initialize payment', 'error');
  return;
}
...
showMessage('fundMessage', 'Unable to connect to payment service', 'error');
}
}
...
if (data.success) {
  ...
  data.providers.forEach(provider => {
    select.innerHTML += `<option value="${escapeHtml(provider)}">${escapeHtml(provider)}</option>`;
  });
}
```
...
const plansList = document.getElementById('airtimePlansList');
...
try {
const response = await fetch(`${API_BASE}/airtime/plans?provider=${encodeURIComponent(provider)}`);
const data = await response.json();
...
if (data.success) {
  ...
  data.plans.forEach(plan => {
    html += `
      <div class="plan-item">
        <div class="plan-info">
          <h4>${escapeHtml(plan.provider)}</h4>
          <p>${escapeHtml(plan.description || '')}</p>
        </div>
        <button class="btn btn-primary plan-buy-btn"
          onclick="buyAirtime(${Number(plan.id)}, ${Number(plan.price)}, this)">
          ₦${Number(plan.price).toLocaleString('en-NG')}
        </button>
      </div>`;
  });

  plansList.innerHTML = html || '<p>No airtime plans available.</p>';
}
```
...
async function buyAirtime(planId, amount, button) {
if (!token) {
...
showMessage('airtimeMessage', 'Please enter a valid Nigerian phone number, e.g. 08011111111', 'error');
return;
}

amount = parseFloat(amount);

if (currentUser && parseFloat(currentUser.wallet_balance) < amount) {
...
button.textContent = `₦${Number(amount).toLocaleString('en-NG')}`;
}
}
}
...
if (data.success) {
  ...
  showMessage('dataMessage', data.message || 'Failed to load providers', 'error');
}
```

} catch (error) {
console.error('Failed to load data providers:', error);
showMessage('dataMessage', 'Failed to load data providers', 'error');
}
}
...
plansList.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:1rem;">Loading ${escapeHtml(provider)} data plans...</div>`;

try {
const response = await fetch(`${API_BASE}/data/plans?provider=${encodeURIComponent(provider)}`);
const data = await response.json();

```
if (!data.success) {
  plansList.innerHTML = '';
  showMessage('dataMessage', data.message || 'Failed to load data plans', 'error');
  return;
}

if (!data.plans || data.plans.length === 0) {
  plansList.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:1.5rem;">No data plans available for ${escapeHtml(provider)}.</div>`;
  return;
}
...
data.plans.forEach(plan => {
  const price = parseFloat(plan.price).toLocaleString('en-NG', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
  });

  const validity = plan.validity_days
  ...
  html += `
    <div class="plan-item">
      <div class="plan-info">
        ...
        <div class="plan-data-size">${escapeHtml(plan.data_size || 'Data Bundle')}</div>
        <div class="plan-validity">${validity}</div>
      </div>
      <div class="plan-bottom">
        <div class="plan-price">₦${price}</div>
        <button class="btn btn-primary plan-buy-btn"
          onclick="buyData(${Number(plan.id)}, ${Number(plan.price)}, this)">
          Buy
        </button>
      </div>
    </div>`;
...
plansList.innerHTML = '';
showMessage('dataMessage', 'Failed to load data plans', 'error');
}
}

async function buyData(planId, amount, button) {
if (!token) {
...
showMessage('dataMessage', 'Please enter a valid Nigerian phone number, e.g. 08011111111', 'error');
phoneInput.focus();
return;
}
...
showMessage('dataMessage', 'Invalid data plan amount', 'error');
return;
}

if (currentUser && parseFloat(currentUser.wallet_balance) < amount) {
showMessage('dataMessage', `Insufficient wallet balance. You need ₦${amount.toLocaleString('en-NG')}.`, 'error');
return;
}
...
showMessage('dataMessage', 'Processing data purchase. Please wait...', 'info');
...
body: JSON.stringify({ phone_number: cleanPhone, plan_id: planId })
});
...
if (data.success) {
  showMessage('dataMessage', `✓ Data purchase successful! ${data.plan} has been sent to ${data.phone_number}. Reference: ${data.reference}`, 'success');
  await updateWalletBalance();

  if (data.wallet_balance !== undefined && currentUser) {
    currentUser.wallet_balance = parseFloat(data.wallet_balance);
    localStorage.setItem('user', JSON.stringify(currentUser));
  }

  phoneInput.value = '';
} else {
  showMessage('dataMessage', data.message || 'Data purchase failed', 'error');
}
```

} catch (error) {
console.error('Data purchase error:', error);
showMessage('dataMessage', 'Network error. Please try again.', 'error');
} finally {
if (button) {
button.disabled = false;
...
if (data.success) {
  ...
  data.types.forEach(type => {
    select.innerHTML += `<option value="${escapeHtml(type.id)}">${escapeHtml(type.name)}</option>`;
  });
}
```
...
if (!utilityTypeId || !account || !Number.isFinite(amount) || amount <= 0) {
showMessage('utilityMessage', 'Please fill all fields correctly', 'error');
return;
}
...
historyContent.innerHTML = '<p>Loading transactions...</p>';
...
if (data.success && data.transactions && data.transactions.length > 0) {
  ...
  data.transactions.forEach(transaction => {
    ...
    const typeDisplay = isCredit ? 'Credit' : 'Debit';
    const status = transaction.status || 'completed';

    html += `
      <div style="border:1px solid #ddd;border-radius:10px;padding:15px;margin-bottom:12px;background:#fff;">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;">
          <strong>${escapeHtml(transaction.description || '')}</strong>
          <strong>${escapeHtml(amountDisplay)}</strong>
        </div>
        <div style="margin-top:8px;font-size:14px;color:#666;">
          ${escapeHtml(typeDisplay)} • ${escapeHtml(status)}
        </div>
        <div style="margin-top:5px;font-size:13px;color:#888;">
          ${escapeHtml(date)}
        </div>
        <button class="btn btn-primary" onclick="downloadTransactionReceipt(${Number(transaction.id)})" style="margin-top:10px;width:100%;">
          🧾 Download Receipt
        </button>
      </div>`;
  });
  ...
  historyContent.innerHTML = '<p>No transactions yet.</p>';
}
```

} catch (error) {
console.error('Failed to load transaction history:', error);
historyContent.innerHTML = '<p>Failed to load transaction history.</p>';
}
}
...
if (!token) {
showMessage('historyContent', 'Please login first', 'error');
return;
}
...
const transaction = data.transactions.find(t => Number(t.id) === Number(transactionId));
...
const status = transaction.status || 'completed';
const receiptNumber = `DH-${transaction.id}-${Date.now()}`;
...
const receiptHtml = `
  ...
  <head>
    ...
    <title>DataHub Transaction Receipt</title>
    <style>
      body{font-family:Arial,sans-serif;background:#f5f5f5;padding:30px;}
      .receipt{max-width:500px;margin:auto;background:#fff;padding:30px;border-radius:10px;box-shadow:0 2px 10px rgba(0,0,0,.1);}
      ...
      .label{color:#666;}.value{font-weight:bold;text-align:right;}.amount{font-size:24px;color:#1e3c72;}
      .footer{text-align:center;margin-top:25px;color:#777;font-size:13px;}
      .print-btn{display:block;margin:20px auto 0;padding:10px 18px;border:0;border-radius:5px;background:#1e3c72;color:#fff;cursor:pointer;}
  ...
  <body>
    <div class="receipt">
      ...
      <div class="row"><span class="label">Transaction ID</span><span class="value">${escapeHtml(String(transaction.id))}</span></div>
      <div class="row"><span class="label">Description</span><span class="value">${escapeHtml(transaction.description || '')}</span></div>
      <div class="row"><span class="label">Type</span><span class="value">${escapeHtml(type)}</span></div>
      ...
      <div class="footer">Thank you for using DataHub.<br>Keep this receipt for your records.</div>
...
link.href = url;
link.download = `DataHub-Receipt-${transaction.id}.html`;
...
alert('Unable to create receipt. Please try again.');
}
}
...
if (data.success) {
  ...
  showMessage('fundMessage', data.message || 'Payment verification failed.', 'error');
}
```

} catch (error) {
console.error('Payment verification error:', error);
showMessage('fundMessage', 'Unable to verify payment. Please check your transaction history.', 'error');
} finally {
...
window.loadAirtimePlans = loadAirtimePlans;
window.loadDataPlans = loadDataPlans;
