...
function selectService(service) {
  ...
  if (airtimeSection) airtimeSection.style.display = 'none';
  if (dataSection) dataSection.style.display = 'none';
  if (utilitySection) utilitySection.style.display = 'none';

  if (service === 'airtime') {
    if (airtimeSection) airtimeSection.style.display = 'block';
...
async function loadAirtimeProviders() {
  try {
    ...
    if (data.success) {
      const select =
        document.getElementById('airtimeProvider');
...
async function loadAirtimePlans() {
  const provider =
    document.getElementById('airtimeProvider').value;
...
async function buyAirtime(
  ...
  try {
    ...
    if (button) {
       ...
       * Airtime buttons normally display their amount.
       */
      button.textContent =
...
async function loadDataProviders() {
  try {
    ...
    const select =
      document.getElementById('dataProvider');
...
async function loadDataPlans() {
  ...
  plansList.innerHTML = `
    <div style="
      ...
      padding: 1rem;
    ">
      Loading ${escapeHtml(provider)} data plans...
    </div>
  `;
...
async function buyData(
  ...
  // ==========================================
  // IMPORTANT:
  // Only the button that was clicked is changed.
  // ==========================================
  ...
  try {
    ...
    if (data.success) {
      ...
      showMessage(
        'dataMessage',
        `✓ Data purchase successful! ${data.plan} has been sent to ${data.phone_number}. Reference: ${data.reference}`,
        'success'
      );
    ...
    // Restore ONLY the clicked button
    if (button) {
      button.disabled = false;
        ...
        html += `
          ...
          ">
            ...
            <button
              ...
              onclick='downloadTransactionReceipt(${JSON.stringify(transaction)})'
    ...
    <body>
      <div class="receipt">
        ...
        <div class="row">
          ...
          <span class="value">${escapeHtml(String(transaction.id))}</span>
        </div>
        ...
        <div class="row">
          ...
          <span class="value amount">₦${escapeHtml(amount)}</span>
        </div>
        ...
        <div class="footer">
          Thank you for using DataHub.<br>
...
async function handlePaymentCallback() {
  ...
  // Extract reference from Paystack URL
